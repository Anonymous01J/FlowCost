...
import { calcLabor } from '../calculations';
...
function newRow(): LaborRow {
  return { id: Math.random().toString(36).slice(2), position: '', payType: 'hora', amountUSD: 0, timeQuantity: 1 };
}

const timeLabel: Record<string, string> = {
  hora: 'Horas', dia: 'Días', semana: 'Semanas', mes: 'Meses',
};
    ...
    <Card borderColor="$borderColor" borderWidth={1} borderRadius="$4" padding="$3"
      ...
      <XStack gap="$2">
        <YStack flex={1}>
          ...
          <SelectTrigger label={selectedPayTypeLabel} onPress={() => setPayTypeSheetOpen(true)} />
        </YStack>
        <YStack flex={1}>
          <InputCustom
            label="Monto USD"
            ...
            onChangeValue={(n) => onUpdate({ amountUSD: n })}
          />
        </YStack>
      </XStack>
      ...
      <XStack gap="$2">
        <YStack flex={1}>
          <InputCustom
            label={timeLabel[row.payType]}
            variant="decimal"
            placeholder="1"
            value={timeDisplay}
            onChangeText={setTimeDisplay}
            onChangeValue={(n) => onUpdate({ timeQuantity: n })}
          />
        </YStack>
        <YStack flex={1}>
          <SizableText size="$2" color="$colorSubtitle" fontWeight="500" marginBottom="$1">
      ...
      <OptionSheet open={payTypeSheetOpen} onOpenChange={setPayTypeSheetOpen}
        options={PAY_TYPE_OPTIONS} value={row.payType}
        onSelect={(val) => { onUpdate({ payType: val as LaborRow['payType'] }); setPayTypeSheetOpen(false); }}
        title="Tipo de Pago" />
    </Card>
  );
}
  ...
  return (
    <YStack gap="$4">
      ...
      <Button onPress={addRow} borderColor="$blue7" borderWidth={1} borderStyle="dashed"
        ...
        <SizableText color="$blue9">Agregar Personal</SizableText>
      </Button>
