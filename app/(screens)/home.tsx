...
function StatCard({ icon, label, value, bg, borderColor }: {
  ...
  return (
    <Card flex={1} padding="$3" gap="$2"
      ...
      <SizableText size="$4" fontWeight="700" color="$color" numberOfLines={1}>{value}</SizableText>
    </Card>
  );
}
...
export default function Home() {
  ...
  const { budgets, loading, addBudget, updateBudget, updateBudgetStatus, deleteBudget } = useBudgets();
  ...
  // Color del buscador según el tema de la app (no del SO)
  ...
  const ListHeader = (
    <YStack gap="$4" paddingBottom="$2">
      ...
      <XStack gap="$3">
        <StatCard icon={<DollarSign size={14} color="$blue9" />}  label="Total USD" value={`$${formatVE(totalUSD, 0)}`}   bg="$blue3"   borderColor="$blue6" />
        <StatCard icon={<TrendingUp size={14} color="$green9" />} label="Total Bs." value={`Bs. ${formatVE(totalBS, 0)}`} bg="$green3"  borderColor="$green6" />
      </XStack>
      <XStack gap="$3">
        <StatCard icon={<CheckSquare size={14} color="$purple9" />} label="Listos"        value={String(listoCount)}        bg="$purple3" borderColor="$purple6" />
        <StatCard icon={<FileText    size={14} color="$orange9" />} label="En Desarrollo" value={String(enDesarrolloCount)} bg="$orange3" borderColor="$orange6" />
      </XStack>

      <XStack
        backgroundColor="$backgroundStrong" borderColor="$borderColor"
      ...
      <XStack alignItems="center" gap="$2">
        ...
        <XStack gap="$2">
          <FilterPill label="Todos"         active={filterStatus === 'all'}            onPress={() => setFilterStatus('all')} />
          <FilterPill label="Listos"        active={filterStatus === 'listo'}          onPress={() => setFilterStatus('listo')} />
          <FilterPill label="En Desarrollo" active={filterStatus === 'en-desarrollo'}  onPress={() => setFilterStatus('en-desarrollo')} />
        </XStack>
      </XStack>
    </YStack>
  );
  ...
  if (loading) {
    return (
      <YStack flex={1} alignItems="center" justifyContent="center" backgroundColor="$background">
        ...
        <SizableText size="$3" color="$colorSubtitle" marginTop="$3">Cargando presupuestos...</SizableText>
      </YStack>
    );
  }
  ...
  return (
    <YStack flex={1} backgroundColor="$background">
      ...
      <Button
        ...
        onPress={() => { setEditingBudget(null); setModalOpen(true); }}
